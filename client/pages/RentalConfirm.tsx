    ...
    <Layout>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-md border-2 border-game-brown border-opacity-10 space-y-4">
          ...
          <div>
            <p className="text-sm text-game-brown">
              Inicio: <strong>{state.startDate ?? "-"}</strong>
            </p>
            <p className="text-sm text-game-brown">
              Fin: <strong>{state.endDate ?? "-"}</strong>
            </p>
            <p className="text-sm text-game-brown">
              Retiro: <strong>{state.method ?? "pickup"}</strong>
            </p>
            {state.method === "delivery" && (
              <p className="text-sm text-game-brown">
                Dirección: <strong>{state.address}</strong>
              </p>
            )}
            <p className="text-sm text-game-brown">
              Pago:{" "}
              <strong>
                {state.payment === "mercado" ? "Mercado Pago" : "Efectivo"}
              </strong>
            </p>
          </div>
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-2xl font-bold text-game-rust">
                ${game?.price}
              </p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => navigate(-1)}
                className="px-4 py-2 rounded-lg border border-game-brown"
              >
                Volver
              </button>
              <button
