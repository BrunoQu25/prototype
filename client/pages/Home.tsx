          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {recentlyAdded.map((game) => (
              <Link key={game.id} to={`/product/${game.id}`}>
                <div className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all transform hover:-translate-y-2 border-2 border-game-sage border-opacity-50 cursor-pointer h-full">
                  {/* Game Image Area */}
                  <div className="bg-gradient-to-br from-game-sage to-green-200 h-48 flex items-center justify-center text-8xl animate-float">
                    {game.image}
                  </div>
                  ...
                  <div className="p-5">
                    ...
                    {/* Category Badge */}
                    <div className="inline-block px-3 py-1 bg-game-sage bg-opacity-10 border border-game-sage rounded-full text-xs font-semibold text-game-sage mb-3">
                      {game.category}
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-game-brown border-opacity-10">
                      <span className="font-bold text-lg text-game-rust">${game.price}/día</span>
                      <ChevronRight className="w-5 h-5 text-game-sage" />
                    </div>
                  </div>
                </div>
              </Link>
